...
def _find_all_marker_paragraphs(section_xml: str, markers: list) -> list:
    """section_xml 안에서 markers 를 포함한 모든 <hp:p>...</hp:p> 의
...
def _patched_build_hwpx(template_path: str, output_path: str, problems: list,
    ...
    # 1) 템플릿 section 미리 읽어 표식 유무 확인 (모든 표식 위치)
    ...
    # 2) 자리표시자 모드: N개 표식에 문제 그룹 N개 1:1 분배
    #    초과 그룹은 마지막 표식에, 남는 표식은 원본 유지.
    groups = _group_problems(problems)
    N, M = len(markers_pos), len(groups)
    assignments = []  # [(start, end, group_indices or None)]
    for i in range(N):
        if M == 0:
...
**다단(Multi-column)도 지원** — 한글에서 `쪽 → 단 → 2단/3단` 설정하면
그 다단 배치도 그대로 적용됩니다. 변환된 문제들이 자동으로 단에 흘러들어갑니다.

샘플 템플릿이 프로젝트 루트에 있습니다:
- `샘플_시험지_템플릿.hwpx` (1단)
- `샘플_시험지_템플릿_2단.hwpx` (2단)
"""
        )
