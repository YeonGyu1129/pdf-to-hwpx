...
{"number": "1", "role": "solution", "segments": [
    ...
    {"type": "formula", "content": "f(1)=2"}
]}
```

규칙:
- `role` 은 반드시 `"solution"`.
- `number` 는 그 풀이가 **설명하는 문제 번호**와 동일하게 (예: 3번 풀이 → `"3"`).
- 풀이가 여러 줄이면 **줄마다 별도 엔트리** (같은 number, 같은 role).
- 본문/보기/박스는 기존대로 일반 엔트리로. 풀이만 `role:"solution"` 으로 분리.
- 풀이 엔트리는 본문 엔트리들보다 **뒤에 배치**해도 number 로 자동 매칭됩니다.
- 사용자가 별도로 요청하지 않으면 풀이는 기존처럼 일반 문단으로 출력합니다.
